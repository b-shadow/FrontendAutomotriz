...
const CitaDetalleModal = ({ cita, onClose }) => {
  const estadoColorMap = {
    PROGRAMADA: 'bg-green-100 text-green-800', PENDIENTE_APROBACION : 'bg-yellow-100 text-yellow-800',
    CANCELADA: 'bg-red-100 text-red-800', FINALIZADA : 'bg-blue-100 text-blue-800',
    NO_SHOW: 'bg-orange-100 text-orange-800',
  }

  const colorClass = estadoColorMap[cita.estado] || 'bg-neutral-100 text-carbon-800'
      ...
      <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-screen overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="bg-neutral-50 p-4 rounded-lg">
            <p className="text-xs text-carbon-500 uppercase font-semibold">Vehículo</p>
          ...
          <div className="bg-neutral-50 p-4 rounded-lg">
            <p className="text-xs text-carbon-500 uppercase font-semibold">Cliente</p>
            <p className="text-lg font-semibold">
              {cita.cliente.nombre_completo || cita.cliente.nombres}
            </p>
            <p className="text-sm text-carbon-600">{cita.cliente.email}</p>
          </div>

          <div className="bg-neutral-50 p-4 rounded-lg">
            <p className="text-xs text-carbon-500 uppercase font-semibold">Estado</p>
            <div className="mt-2">
          ...
          <div className="bg-neutral-50 p-4 rounded-lg">
            <p className="text-xs text-carbon-500 uppercase font-semibold">Duración Total</p>
        ...
        <div className="bg-blue-50 p-4 rounded-lg mb-6">
          ...
          <p className="text-sm text-blue-800"> ? Inicio : {new Date(cita.fecha_hora_inicio_programada).toLocaleString()}
          </p>
        </div>
        ...
        <div className="mb-6">
          ...
          {Object.entries(segmentosPorFecha).map(([fecha, segmentos]) => (
            <div key={fecha} className="mb-4">
              <p className="text-sm font-semibold text-carbon-700 mb-2">
              ...
              <div className="space-y-2">
                {segmentos.map((seg) => (
                  <div key={seg.id} className="border rounded-lg p-3 bg-neutral-50">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="text-right">
                        <p className="text-xs font-semibold text-carbon-500 uppercase">
                          {seg.tipo_segmento}
                        </p>
                      </div>
                    </div>
          ...
          {(!cita.espacios_segmentos || cita.espacios_segmentos.length === 0) && (
            <div className="bg-yellow-50 p-4 rounded-lg text-sm text-yellow-800">
              <AlertTriangle className="inline-block mx-1 text-current" size={20} strokeWidth={2} /> No hay segmentos definidos aún. Backend estará calculando.
            </div>
          )}
        </div>
          ...
          {cita.detalles && cita.detalles.length > 0 ? (
            <div className="space-y-2">
              {cita.detalles.map((detalle, idx) => (
                <div key={idx} className="flex justify-between items-center p-3 bg-neutral-50 rounded-lg text-sm">
                  <div>
            ...
            <div className="bg-neutral-50 p-4 rounded-lg text-sm text-carbon-600">
              <Info className="inline-block mx-1 text-current" size={20} strokeWidth={2} /> Sin servicios registrados
            </div>
          )}
        </div>
        ...
        {cita.observaciones_cliente && (
          <div className="mb-6">
            ...
            <div className="bg-neutral-50 p-3 rounded-lg text-sm text-carbon-700">
              {cita.observaciones_cliente}
            </div>
          </div>
        )}
        ...
        {/* Metadata */}
        <div className="border-t pt-4 text-xs text-carbon-500 space-y-1">
          <p>
