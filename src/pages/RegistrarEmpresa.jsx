...
function RegistrarEmpresa() {
  ...
  const getPlanPrice = (plan) => {
    if (!plan) return 0
    if (typeof plan.precio_usd === 'number') return plan.precio_usd
    if (typeof plan.precio === 'number') return plan.precio
    if (typeof plan.precio_formateado === 'string') {
      const parsed = Number.parseFloat(plan.precio_formateado)
      if (!Number.isNaN(parsed)) return parsed
    }
    if (typeof plan.precio_centavos === 'number') return plan.precio_centavos / 100
    return 0
  }
      ...
      if (result.success && result.empresa && result.empresa.slug) {
        ...
        const errorMsg = !result.success
         ? (result.error || 'Error al confirmar el pago')
          : 'La empresa registrada no tiene un identificador v?lido. Por favor contacta soporte.'
        setError(errorMsg)
      }
    } catch (err) {
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      const result = await authService.crearPagoEmpresa(
        selectedPlan.id,
        {
          nombre: formData.nombre, slug : formData.slug,
          nit: formData.nit,
        },
        {
      ...
      <div className="relative min-h-screen overflow-x-hidden bg-neutral-100 text-carbon-900 transition-colors duration-300 dark:bg-carbon-950 dark:text-white">
        ...
        <div className="relative z-10 pt-24 sm:pt-28 flex items-center justify-center min-h-screen">
          <div className="w-full max-w-2xl mx-auto px-4">
            <div className="rounded-2xl border border-neutral-200 bg-white/80 shadow-2xl backdrop-blur dark:border-white/[0.06] dark:bg-white/[0.03] p-8 sm:p-12">
              <h1 className="text-3xl font-bold text-carbon-900 dark:text-white mb-2 text-center">
                Paso 1: Selecciona un Plan
              </h1>
              <p className="text-carbon-600 dark:text-neutral-300 text-center mb-8">
              ...
              {planesLoading ? (
                <div className="text-center p-8">
                  ...
                  <p className="mt-4 text-carbon-600 dark:text-neutral-300">Cargando planes...</p>
                </div>
              ) : planes.length === 0 ? (
                ...
                <>
                      ...
                      <button
                        ...
                        className={`p-6 rounded-xl border-2 text-left transition-all ${
                          selectedPlan?.id === plan.id ?
                             'border-primary-500 bg-primary-50/50 dark:border-primary-400 dark:bg-primary-500/10 shadow-lg'
                            : 'border-neutral-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.03] hover:border-primary-300 dark:hover:border-primary-400'
                        }`}
                      >
                        <div className="flex justify-between items-start">
                          <div>
                            <h3 className="text-xl font-semibold text-carbon-900 dark:text-white">{plan.nombre}</h3>
                            <p className="text-carbon-600 dark:text-neutral-300 mt-1">{plan.descripcion || 'Sin descripción'}</p>
                          </div>
                          <div className="text-right">
                            <p className="text-2xl font-bold bg-gradient-to-r from-primary-500 to-burgundy-500 bg-clip-text text-transparent">${getPlanPrice(plan).toFixed(2)}</p>
                            <p className="text-sm text-carbon-500 dark:text-neutral-400">/mes</p>
                          </div>
                        </div>
                      </button>
                    ))}
                  ...
                  <div className="flex gap-4">
                    <button
                      onClick={() => navigate('/')}
                      className="flex-1 rounded-xl border-2 border-primary-500/40 bg-white/80 px-6 py-3 text-base font-semibold uppercase tracking-wider text-carbon-800 shadow-sm transition hover:-translate-y-1 hover:border-primary-500 hover:text-primary-600 dark:border-primary-400/30 dark:bg-white/[0.04] dark:text-white dark:hover:border-primary-400 dark:hover:text-primary-400"
                    >
                      Cancelar
                    </button>
                    <button
                      ...
                      onClick={() => setStep(2)}
                      className="flex-1 rounded-xl bg-gradient-to-r from-primary-600 via-primary-500 to-burgundy-600 px-6 py-3 text-base font-bold uppercase tracking-wider text-white shadow-2xl shadow-primary-900/20 transition hover:-translate-y-1 hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Siguiente Paso →
                    </button>
                  </div>
  ...
  if (step === 2) {
    ...
    return (
      <div className="relative min-h-screen overflow-x-hidden bg-neutral-100 text-carbon-900 transition-colors duration-300 dark:bg-carbon-950 dark:text-white">
        ...
        <div className="relative z-10 pt-24 sm:pt-28 flex items-center justify-center min-h-screen">
          <div className="w-full max-w-2xl mx-auto px-4">
            <div className="rounded-3xl border border-neutral-200 bg-white/80 shadow-2xl backdrop-blur dark:border-white/10 dark:bg-white/5 p-8 sm:p-12">
              <h1 className="text-3xl font-bold text-carbon-900 dark:text-white mb-2 text-center">
                Paso 2: Datos de Empresa y Usuario
              </h1>
              <p className="text-carbon-600 dark:text-neutral-300 text-center mb-6">
                Plan seleccionado: <span className="font-semibold bg-gradient-to-r from-primary-500 to-burgundy-500 bg-clip-text text-transparent">{selectedPlan.nombre}</span>
              </p>
              <form className="space-y-6">
                {/* Empresa Section */}
                <div className="border-b border-neutral-200 dark:border-white/[0.08] pb-6">
                  <h3 className="text-lg font-semibold text-carbon-900 dark:text-white mb-4">Datos de la Empresa</h3>
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-medium text-carbon-900 dark:text-white mb-2">Nombre de la Empresa</label>
                      <input
                        type="text"
                        name="nombre"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-carbon-900 dark:text-white mb-2">Slug (URL identificador)</label>
                      <input
                        type="text"
                        name="slug"
                        value={formData.slug}
                        ...
                        required
                      />
                      <p className="text-xs text-carbon-500 dark:text-neutral-400 mt-1">Solo letras minúsculas, números y guiones</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-carbon-900 dark:text-white mb-2">NIT</label>
                      <input
                        type="text"
                        name="nit"
                        value={formData.nit}
                ...
                {/* Usuario Section */}
                <div>
                  <h3 className="text-lg font-semibold text-carbon-900 dark:text-white mb-4">Usuario Administrador</h3>
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-carbon-900 dark:text-white mb-2">Nombres</label>
                        <input
                          type="text"
                          name="nombres"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-carbon-900 dark:text-white mb-2">Apellidos</label>
                        <input
                          type="text"
                          name="apellidos"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-carbon-900 dark:text-white mb-2">Email</label>
                      <input
                        type="email"
                        name="email"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-carbon-900 dark:text-white mb-2">Contraseña</label>
                      <input
                        type="password"
                        name="password"
  ...
  if (step === 3) {
    const cardError = validateCardData()
    return (
      <div className="relative min-h-screen overflow-x-hidden bg-neutral-50 text-carbon-900 transition-colors duration-300 dark:bg-[#0b0720] dark:text-white">
        {renderBackground()}
        {renderNavbar()}
        ...
        <div className="relative z-10 pt-24 sm:pt-28 flex items-center justify-center min-h-screen">
          <div className="w-full max-w-2xl mx-auto px-4">
            <div className="rounded-3xl border border-neutral-200 bg-white/80 shadow-2xl backdrop-blur dark:border-white/10 dark:bg-white/5 p-8 sm:p-12">
              ...
              {/* Resumen del pedido */}
              <div className="bg-neutral-50/50 dark:bg-white/[0.03] rounded-xl p-6 mb-6 space-y-3 border border-neutral-200 dark:border-white/[0.06]">
                <div className="flex justify-between">
                  <span className="text-carbon-700 dark:text-neutral-300">Empresa:</span>
                  <span className="font-semibold text-carbon-900 dark:text-white">{formData.nombre}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-carbon-700 dark:text-neutral-300">Plan:</span>
                  <span className="font-semibold text-carbon-900 dark:text-white">{selectedPlan.nombre}</span>
                </div>
                <div className="flex justify-between pt-3 border-t border-neutral-200 dark:border-white/10">
                  <span className="text-carbon-900 dark:text-white font-semibold">Total:</span>
                  <span className="text-2xl font-bold bg-gradient-to-r from-primary-500 to-burgundy-500 bg-clip-text text-transparent">${getPlanPrice(selectedPlan).toFixed(2)}</span>
                </div>
              </div>

              {/* Formulario de tarjeta */}
              <div className="bg-white dark:bg-white/[0.03] border-2 border-neutral-200 dark:border-white/[0.06] rounded-xl p-6 mb-6 space-y-4">
                <div>
                  <label className="block text-sm font-medium text-carbon-900 dark:text-white mb-2">Nombre del titular</label>
                  <input
                    type="text"
                    name="cardholderName"
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-2xl p-4 mb-6">
                <p className="text-sm font-semibold text-blue-900 dark:text-blue-200 mb-2"><CreditCard className="inline-block mx-1 text-current" size={20} strokeWidth={2} /> Datos de Prueba de Stripe:</p>
    ...
    if (successData && successData.empresa) {
      ...
      return (
        <div className="relative min-h-screen overflow-x-hidden bg-neutral-50 text-carbon-900 transition-colors duration-300 dark:bg-[#0b0720] dark:text-white">
          ...
          <div className="relative z-10 pt-24 sm:pt-28 flex items-center justify-center min-h-screen">
            <div className="w-full max-w-2xl mx-auto px-4">
              <div className="rounded-3xl border border-neutral-200 bg-white/80 shadow-2xl backdrop-blur dark:border-white/10 dark:bg-white/5 p-8 sm:p-12 text-center">
                <div className="text-6xl mb-4"><CheckCircle className="inline-block mx-1 text-current" size={20} strokeWidth={2} /></div>
                <h1 className="text-3xl font-bold text-carbon-900 dark:text-white mb-2">
                  ¡Empresa Registrada Exitosamente!
                </h1>
                <p className="text-carbon-600 dark:text-neutral-300 mb-8">
                  Tu empresa "{empresa.nombre}" está lista para usar. Serás redirigido al login en unos momentos...
                </p>

                <div className="bg-neutral-50/50 dark:bg-white/[0.03] border border-neutral-200 dark:border-white/[0.06] rounded-xl p-6 mb-6 space-y-3 text-left">
                  <div>
                    <p className="text-sm text-carbon-600 dark:text-neutral-400">Empresa:</p>
                  ...
                  <div>
                    <p className="text-sm text-carbon-600 dark:text-neutral-400">Plan:</p>
                    <p className="font-semibold text-carbon-900 dark:text-white">{selectedPlan.nombre}</p>
                  </div>
                </div>
