import { CheckCircle, Settings, AlertTriangle, Pencil, Pause, Play } from 'lucide-react';
  ...
  const handleGuardarServicio = async (formData) => {
    ...
    try {
      if (servicioEnEdicion) {
        ...
        setSuccessMessage(<><CheckCircle className="inline-block mx-1 text-current" size={20} strokeWidth={2} /> Servicio actualizado correctamente</>)
      } else {
        // Crear
        await serviciosCatalogoService.crearServicio(tenantSlug, formData)
        setSuccessMessage(<><CheckCircle className="inline-block mx-1 text-current" size={20} strokeWidth={2} /> Servicio registrado correctamente</>)
      }
      handleCloseModal()
  ...
  const handleCambiarEstado = async () => {
    ...
    try {
      ...
      await serviciosCatalogoService.cambiarEstadoServicio(tenantSlug, servicioParaEstado.id, {
        activo: nuevoEstado, motivo : motivoEstado,
      })
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-carbon-900 dark:text-white flex items-center gap-2">
            <Settings className="inline-block mx-1 text-current" size={20} strokeWidth={2} /> Catálogo de Servicios
          </h1>
        ...
        <div className="flex gap-4 items-end">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-4 py-2 border border-neutral-300 dark:border-white/[0.08] rounded-lg text-carbon-700 dark:text-neutral-300 hover:bg-neutral-50 dark:hover:bg-carbon-700 transition-colors font-medium"
            >
              Limpiar
            </button>
            <button
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12">
          <div className="text-center">
            ...
            <p className="text-carbon-600 dark:text-neutral-400">Cargando servicios...</p>
          </div>
        </div>
          ) : servicios.length === 0 ? (
        <div className="bg-white dark:bg-carbon-800 rounded-lg shadow p-8 text-center">
          <div className="text-4xl mb-3"><Settings className="inline-block mx-1 text-current" size={20} strokeWidth={2} /></div>
        ...
        <div className="bg-white dark:bg-carbon-800 rounded-lg shadow overflow-hidden">
          <table className="w-full">
            <thead className="border-b border-neutral-200 dark:border-white/[0.08]">
              <tr>
                <th className="px-6 py-3 text-left text-sm font-semibold text-carbon-900 dark:text-white">Código</th>
                <th className="px-6 py-3 text-left text-sm font-semibold text-carbon-900 dark:text-white">Nombre</th>
                <th className="px-6 py-3 text-left text-sm font-semibold text-carbon-900 dark:text-white">Descripción</th>
                <th className="px-6 py-3 text-left text-sm font-semibold text-carbon-900 dark:text-white">Tiempo (min)</th>
                <th className="px-6 py-3 text-left text-sm font-semibold text-carbon-900 dark:text-white">Precio (Bs.)</th>
              ...
              {servicios.map((servicio, idx) => (
                ...
                >
                  <td className="px-6 py-4">
                    <span className="font-mono text-sm text-carbon-900 dark:text-white">{servicio.codigo}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="font-semibold text-carbon-900 dark:text-white">{servicio.nombre}</span>
                  </td>
                  <td className="px-6 py-4 text-carbon-700 dark:text-neutral-300 text-sm">
                    {servicio.descripcion || '-'}
                  </td>
                  <td className="px-6 py-4 text-carbon-700 dark:text-neutral-300">
                    {servicio.tiempo_estandar_min} min
                  </td>
                  <td className="px-6 py-4">
                    <span className="font-medium text-carbon-900 dark:text-white">
                      ...
                      {canEdit && (
                        <>
                          ...
                          {canChangeStatus && (
                            ...
                            >
                              {servicio.activo ? <><Pause className="inline-block mx-1 text-current" size={20} strokeWidth={2} /> Desactivar</> : <><Play className="inline-block mx-1 text-current" size={20} strokeWidth={2} /> Activar</>}
                            </button>
                          )}
                        </>
                      )}
                      {!canEdit && <span className="text-carbon-500 text-xs">Solo lectura</span>}
                    </div>
                  </td>
                </tr>
              ))}
          ...
          <div className="bg-white dark:bg-carbon-800 rounded-lg shadow-xl max-w-sm w-full">
            <div className="px-6 py-4 border-b border-neutral-200 dark:border-white/[0.08]">
              <h2 className="text-xl font-semibold text-carbon-900 dark:text-white">
                {servicioParaEstado.activo ? <><Pause className="inline-block mx-1 text-current" size={20} strokeWidth={2} /></> : <><Play className="inline-block mx-1 text-current" size={20} strokeWidth={2} /></>} Cambiar Estado
              </h2>
            </div>
            <div className="p-6 space-y-4">
              <p className="text-carbon-700 dark:text-neutral-300">
                ¿Deseas cambiar el estado del servicio{' '}
                <strong className="text-carbon-900 dark:text-white">{servicioParaEstado.nombre}</strong> a{' '}
